  ...
  return (
    <div key={category.id} className="relative">
      <Link href={category.url} target="_blank">
        ...
        <div
          className={`font-DMSans flex flex-col md:items-start items-center absolute bottom-12 left-[50%] md:left-12 md:translate-x-0 translate-x-[-50%] text-white ${custom}`}
        >
          <p className={`md:text-[3rem] font-semibold ${customText}`}>{category.title}</p>
          <Button variant={'secondary'} className={`w-24 md:w-36 ${customBtn}`}>
            {btnLabel}{' '}
          </Button>
        </div>
      </Link>
