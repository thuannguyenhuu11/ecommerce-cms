...
const {
  ...
  TemplatePreviews,
  Payments,
} = defineOneEntry('https://thuannguyenec11.oneentry.cloud', { token: process.env.ONEENTRY_TOKEN, langCode: 'en' });
import nodemailer from 'nodemailer';
import Mail from 'nodemailer/lib/mailer';

//Get all data from a page url
...
export async function postFormData(data: any) {
  ...
  try {
    await Promise.all([sendMailPromise(confirmationEmailOptions), sendMailPromise(orderEmailOptions)]);
